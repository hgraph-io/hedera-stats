...
- The data-seed migrations `025-metric_descriptions.sql` and `014-hbar_total_supply_seed.sql`. `ecosystem.metric` and `ecosystem.metric_description` are publisher-controlled — the publisher owns/updates their rows and they are replicated to subscribers — so the migrations must not seed them (and a seed would clash with `copy_data` on a replicating subscriber). `001-init.sql` still creates the table shells. The seed SQL is kept as publisher references to run on the publisher: `src/metric_descriptions.sql` and `src/metrics/hbar-defi/hbar_total_supply.sql`
