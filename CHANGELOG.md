...
- The `ecosystem.metric_description` seed migration (`025-metric_descriptions.sql`). `metric_description` is publisher-controlled — the publisher owns and updates its rows and they are replicated to subscribers — so the migrations must not seed it. `001-init.sql` still creates the table shell, and `src/metric_descriptions.sql` is kept as a publisher reference (run it on the publisher to populate the descriptions)
