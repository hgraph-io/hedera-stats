...
- Migration `083-current_nft_metrics.sql`: the three point-in-time NFT functions `current_total_nfts`, `current_nft_holders`, `current_nft_market_cap`, promoted from `src/metrics/legacy`. `014-load_metrics_beta.sql` calls all three and no migration defined them — a `LANGUAGE plpgsql` body is only syntax-parsed at CREATE time, not name-resolved, so `check_function_bodies` let `014` apply and `load_metrics_beta` would have failed at call time on any database lacking the pre-migration hand-apply. `083` landing after `014` is fine for the same reason (plpgsql resolves callees when the procedure runs)

- Separate PUBLIC migration set (`src/public-migrations/`) for the `public`-schema last-24h matviews `transactions_last_24hrs` and `contract_transactions_last_24hrs` (+ SELECT grants to the readonly role). Applied as the `<db>_owner` role (owns `public`, which `ecosystem_owner` cannot write) and tracked separately in `public.schema_migrations`. Run via the `mainnet-public`/`testnet-public` compose services after the ecosystem set. These are materialized views: not replicated to subscribers and needing a scheduled REFRESH on each node (cron). `migrate.sh` gained a `SCHEMA_MIGRATIONS_TABLE` override to support the second tracking table
- Grants migration: grant `USAGE`/`SELECT`/`EXECUTE` on the `ecosystem` schema to the per-network readonly role (`<db>_readonly`), plus `ALTER DEFAULT PRIVILEGES` so later migrations' objects are auto-granted. Consumer/API roles inherit from readonly; without these grants the api role can't see the schema and Hasura (introspecting as api) reports ecosystem functions/tables as untrackable. `migrate.sh` derives the readonly role from `PGDATABASE` (override with `READONLY_ROLE`) and passes it as the `readonly_role` psql var
- Promoted `active_nft_account_cohorts` (materialized view + its functions and helpers) from `src/metrics/legacy` into a migration. It was tracked in Hasura but deployed nowhere after an earlier move; it is a publisher-side matview (reads mirror-node tables, needs a scheduled REFRESH on the publisher, not replicated to subscribers)
- Migrated the tracked legacy `ecosystem` objects into this repo as migrations: the `transaction_count_by_type` table + `transactions_by_type`, and the per-period query functions (`transactions_per_period`, `associated_transactions_per_period`, `transaction_fees_per_period`, `associated_transaction_fees_per_period`, `fungible_token_transfers_per_period`, `non_fungible_token_transfers_per_period`, `smart_contract_calls_per_period`, `hcs_messages_per_period`, `active_accounts_per_period`, `nft_holders_per_period`). These are exposed via Hasura, so the subscriber needs them; the `transaction_count_by_type` table also lets the schema-wide `ecosystem_pub` publication replicate without erroring. Fixed the eight `*_per_period` functions that emitted a stray `description` column (`ecosystem.metric` has only name/period/timestamp_range/total) — they were return-type-mismatched against the current 4-column metric
...
- Renumbered the migrations to be gapless (`001`–`082`). Earlier iterations left gaps where seed migrations were removed; since this set has never been released, the files were compacted into a contiguous sequence in dependency order
...
- `src/metric_descriptions.sql` and `src/metrics/hbar-defi/hbar_total_supply.sql` are retained as publisher references, and `src/jobs/pg_cron_metrics.sql` as the publisher-only cron schedule; none are loaded at runtime on a subscriber. The rest of the `src/metrics/`/`src/jobs/` source tree was removed (see Removed) — `src/migrations/` is the only copy of each live object
...
- `avg_usd_conversion` set `CURLOPT_TIMEOUT`/`CURLOPT_CONNECTTIMEOUT` (ambiguous seconds-vs-ms) via `http_set_curlopt()` and never called `http_reset_curlopt()`. Both are session-scoped in pgsql-http, so the setting leaked into every other `http_get()` on the same backend — including `network_tvl` and `stablecoin_marketcap`, which set no timeout of their own — and was the root cause of the recurring `PostgresCronJobFailed` alert noise from the two DeFiLlama fetch jobs. Migration `084-defillama_http_timeout_fix.sql`: switches `avg_usd_conversion` to the unambiguous `_MS` curlopt names and resets after its own calls; gives both DeFiLlama fetchers an explicit 10s timeout of their own

### Removed

- Migration `085-drop_load_daily_metrics.sql`: drops `ecosystem.load_daily_metrics()`, the pre-rename loader superseded by `load_metrics_day`. It was still scheduled on the mainnet publisher as pg_cron job 9 (`call ecosystem.load_daily_metrics()`, no explicit jobname) even after its metrics array was emptied out, so it failed every run with "cannot determine type of empty array" — surfacing as a daily `PostgresCronJobFailed` alert that auto-resolved within the 5-minute window regardless of the job's actual health. The cron job has been unscheduled on proxy-1 mainnet; this migration removes the now-orphaned function itself
- The duplicated `src/metrics/**` and `src/jobs/*.sql` source tree (67 files). These were retained as the "readable source" the baseline migrations were generated from, but nothing kept them in sync with the migrations that actually apply, so they were drift waiting to happen and a standing trap (editing one has no effect on any database). Every object defined in the removed files was verified to also be defined in `src/migrations/` first. `src/migrations/` is now the only copy of each live `ecosystem` object. Kept: `src/metric_descriptions.sql` and `src/metrics/hbar-defi/hbar_total_supply.sql` (publisher references), `src/jobs/pg_cron_metrics.sql` (publisher-only cron), and `src/metrics/legacy/` (the promotion queue of publisher objects that still have no migration)
- Dead job scripts (14 files): `src/jobs/temp/` (`load_metrics_init_temp_1`–`6`, one-off backfill scratch), `src/jobs/legacy/` (`load_metrics`, `load_hourly_metrics`, `load_daily_metrics`, `load_init_metrics`, and a `pg_cron.sql` still carrying `<database_name>` placeholders — all superseded by `load_metrics_{hour,day,init}` and `pg_cron_metrics.sql`), and `src/jobs/init/` (`reload_transaction_metrics`, plus `avg_usd_conversion`/`avg_usd_conversion_minute` duplicates of the migrated function). Nothing in the migrations, runner, compose files, or docs referenced any of them
...
- The data-seed migrations for `metric_description` and `hbar_total_supply`. `ecosystem.metric` and `ecosystem.metric_description` are publisher-controlled — the publisher owns/updates their rows and they are replicated to subscribers — so the migrations must not seed them (and a seed would clash with `copy_data` on a replicating subscriber). `001-init.sql` still creates the table shells. The seed SQL is kept as publisher references to run on the publisher: `src/metric_descriptions.sql` and `src/metrics/hbar-defi/hbar_total_supply.sql`
