...
- Migration-driven first-boot bring-up: `src/migrations/migrate.sh` is mounted into `/docker-entrypoint-initdb.d` and applies all migrations (mirror-node types, schema, metrics) on first container start; no separate init wrapper script
...
- Tracked migration runner (`src/migrations/migrate.sh`) with an `ecosystem.schema_migrations` table: applies unapplied `NNN-*.sql` once each, in filename order. Runs during first-boot init and standalone against a live subscriber (`docker compose exec stats-db bash /sql/migrations/migrate.sh`) to apply changes without recreating the volume. Connects over the Unix socket via libpq `PG*` env vars (`PGHOST` defaults to `/var/run/postgresql`), so it needs no password in the container (`local` trust auth); the vars can be overridden to target a database over TCP
...
- Per-network deploy wrapper (`src/migrations/deploy.sh <mainnet|testnet> [--run]`) for applying the migrations into a co-located network mirror-node database. Connects over the Unix socket (peer auth as the postgres OS user, no password) and owns objects as the per-network `hedera_<net>_ecosystem_owner` group role via `PGOPTIONS`. mainnet → port 5433 / `hedera_mainnet`; testnet → port 5432 / `hedera_testnet`. Requires the `timestamp9` and `http` extensions to be pre-installed in the target database

### Removed

- `src/up.sql` and `src/metrics/setup/up.sql` — superseded by `src/migrations/001-init.sql`
- `docker/postgres/init/` (the `01-init.sh` wrapper and `00-mirror-node-types.sql`). Bring-up is now migrations-only: mirror-node types and schema moved into `001-init.sql`, and `migrate.sh` is the first-boot entrypoint. Extension creation was removed entirely — `timestamp9` and `http` are external prerequisites (the migrations are pure schema SQL)
