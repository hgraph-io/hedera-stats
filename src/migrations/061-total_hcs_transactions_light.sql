-- 061-total_hcs_transactions_light.sql — baseline migration generated from src/metrics/transactions/light/total_hcs_transactions_light.sql.
-- Ordered so every ecosystem.* function it references already exists
-- (applies with check_function_bodies on).

BEGIN;

-- Total HCS Transactions LIGHT !!
CREATE OR REPLACE FUNCTION ecosystem.total_hcs_transactions (
  period TEXT,
...
WITH prior AS (
  SELECT COALESCE(
           (SELECT m.total
            ...
            WHERE m.name = 'total_hcs_transactions'
              AND m.period = period
