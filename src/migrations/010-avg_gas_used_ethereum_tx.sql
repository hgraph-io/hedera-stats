-- 010-avg_gas_used_ethereum_tx.sql — baseline migration generated from src/metrics/evm/avg_gas_used_ethereum_tx.sql.
-- Ordered so every ecosystem.* function it references already exists
-- (applies with check_function_bodies on).

BEGIN;

create or replace function ecosystem.avg_gas_used_ethereum_tx(
    period text,
...
with gas_transactions as (
    ...
    and cr.gas_used > 0
    and t.type = 50 -- ethereumTransaction
),
gas_per_period as (
    select
