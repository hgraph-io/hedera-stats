-- 037-new_hts_transactions.sql — baseline migration generated from src/metrics/transactions/new_hts_transactions.sql.
-- Ordered so every ecosystem.* function it references already exists
-- (applies with check_function_bodies on).

BEGIN;

-- New HTS transactions
CREATE OR REPLACE FUNCTION ecosystem.new_hts_transactions (
  period TEXT,
