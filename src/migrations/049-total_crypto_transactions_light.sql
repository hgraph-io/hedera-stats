-- 049-total_crypto_transactions_light.sql — baseline migration generated from src/metrics/transactions/light/total_crypto_transactions_light.sql.
...
WITH prior AS (
  SELECT COALESCE(
           (SELECT m.total
            ...
            WHERE m.name = 'total_crypto_transactions'
              AND m.period = period
...
all_entries AS (
  ...
  WHERE t.consensus_timestamp >= start_timestamp
    ...
    AND t.type IN (10, 11, 12, 13, 14, 15, 48, 49)
  GROUP BY 1
),
period_series AS (
