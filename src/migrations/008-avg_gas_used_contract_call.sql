-- 008-avg_gas_used_contract_call.sql — baseline migration generated from src/metrics/evm/avg_gas_used_contract_call.sql.
-- Ordered so every ecosystem.* function it references already exists
-- (applies with check_function_bodies on).

BEGIN;

create or replace function ecosystem.avg_gas_used_contract_call(
    period text,
