-- 019-load_metrics_month.sql — baseline migration generated from src/jobs/load_metrics_month.sql.
...
----------------------------------------------------
-- LOAD METRICS MONTH / HEDERASTATS.com / HGRAPH.com
-- Automates upsert of metrics into ecosystem.metric
----------------------------------------------------

create or replace procedure ecosystem.load_metrics_month()
language plpgsql
as $$
declare
    periods constant text[] := array['month'];      -- Month (the period for this job)
