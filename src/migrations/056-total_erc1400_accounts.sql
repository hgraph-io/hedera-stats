-- 056-total_erc1400_accounts.sql — baseline migration generated from src/metrics/evm/total_erc1400_accounts.sql.
-- Ordered so every ecosystem.* function it references already exists
-- (applies with check_function_bodies on).

BEGIN;

-- Total accounts holding ERC-1400 tokens
-- Rolling total of unique accounts holding an ERC-1400 token with a positive balance
...
WITH
-- One row per holder: the first time the account associated with any ERC-1400
-- token it currently holds with a positive balance. ERC-1400 partition events
-- are not uniformly populated in erc.token_transfer, so holders are sourced
...
holder_first_association AS (
    ...
    WHERE t.contract_type = 'ERC_1400'
      AND ta.balance > 0
