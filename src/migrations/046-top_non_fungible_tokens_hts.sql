-- 046-top_non_fungible_tokens_hts.sql — baseline migration generated from src/metrics/non-fungible-tokens/top_non_fungible_tokens_hts.sql.
-- Ordered so every ecosystem.* function it references already exists
-- (applies with check_function_bodies on).

BEGIN;

-- Top 50 Non-Fungible Token Collections (HTS) by Composite Score with Concentration Filter
-- 
-- This function ranks HTS (Hedera Token Service) native NFT collections based on a weighted composite score:
...
CREATE OR REPLACE FUNCTION ecosystem.top_non_fungible_tokens_hts(
    window_hours integer DEFAULT 72,
    result_limit integer DEFAULT 50,
    exclusion_threshold numeric DEFAULT 1.0  -- Default: disabled (1.0 = 100%, no filtering)
)
RETURNS SETOF ecosystem._top_non_fungible_tokens_hts
LANGUAGE plpgsql
AS $$
BEGIN
    RETURN QUERY
    ...
    nft_transactions AS (
        -- Extract all HTS NFT transactions in time window
        SELECT 
...
COMMENT ON FUNCTION ecosystem.top_non_fungible_tokens_hts IS 
'Ranks top HTS (Hedera Token Service) native NFT collections using composite score (60% volume + 40% transactions). Concentration filter available (default: disabled with threshold=1.0). Set exclusion_threshold < 1.0 to filter collections where top 5 accounts contribute >threshold of transactions.';
