...
values
    ...
    ('top_non_fungible_tokens_erc', 'Ranks top NFT collections by composite score combining sales volume (60%) and transaction activity (40%) with optional concentration filtering.', 'Analyzes NFT collections in a rolling time window (default: 72 hours) and ranks them using a weighted composite score: (normalized_volume × 0.6) + (normalized_transactions × 0.4). Sales volume is calculated by extracting HBAR amounts from crypto_transfer records associated with NFT transactions, excluding treasury transfers, and allocating proportionally for multi-NFT transactions. Min-max normalization scales both metrics to [0,1] range for fair comparison. Optional concentration filter (disabled by default with threshold=1.0) excludes collections where top 5 most active accounts contribute >80% of transactions, using optimized window functions (ROW_NUMBER OVER PARTITION BY) for sub-second performance. Returns rank, token_id, collection_name, sales_volume_hbar, transaction_count, unique_accounts, normalized metrics, composite_score, contribution breakdown, and concentration_ratio. Function signature: top_non_fungible_tokens_erc(window_hours INT DEFAULT 72, result_limit INT DEFAULT 50, exclusion_threshold NUMERIC DEFAULT 1.0). Methodology: https://docs.hgraph.com/hedera-stats-wip/top-50-non-fungible-tokens'),
