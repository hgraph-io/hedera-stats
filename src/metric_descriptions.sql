...
values
    ...
    ('avg_gas_used', 'Average gas consumed per gas-consuming transaction within a given period.', 'Calculates the average gas usage across all successful transactions with gas consumption (contract_result with transaction_result = 22) during the specified time period. Includes all transaction types. Returns the mean gas consumed per transaction as an integer.'),
    ('avg_gas_used_contract_call', 'Average gas consumed per contract call transaction (type 7) within a given period.', 'Calculates the average gas usage for successful contractCall transactions (type 7) during the specified time period. Returns the mean gas consumed per transaction as an integer.'),
    ('avg_gas_used_ethereum_tx', 'Average gas consumed per Ethereum transaction (type 50) within a given period.', 'Calculates the average gas usage for successful ethereumTransaction transactions (type 50) during the specified time period. These are EVM transactions bridged from Ethereum tooling. Returns the mean gas consumed per transaction as an integer.'),
    ('avg_gas_used_contract_create', 'Average gas consumed per contract creation transaction (type 8) within a given period.', 'Calculates the average gas usage for successful contractCreateInstance transactions (type 8) during the specified time period. Contract deployments typically consume significantly more gas. Returns the mean gas consumed per transaction as an integer.'),
    -- NFTs
    ...
    ('top_non_fungible_tokens_erc', 'Ranks top ERC-721 smart contract NFT collections by composite score combining sales volume (60%) and transaction activity (40%) with optional concentration filtering.', 'Analyzes ERC-721 smart contract NFT collections (erc.token where contract_type = ERC_721) in a rolling time window (default: 72 hours) and ranks them using a weighted composite score: (normalized_volume × 0.6) + (normalized_transactions × 0.4). Sales volume is calculated by extracting HBAR amounts from crypto_transfer records associated with ERC-721 transfer events in erc.nft_transfer table, excluding sender transfers. Min-max normalization scales both metrics to [0,1] range for fair comparison. Optional concentration filter (disabled by default with threshold=1.0) excludes collections where top 5 most active accounts contribute >80% of transactions. Returns rank, token_id, token_evm_address (0x format), collection_name, sales_volume_hbar, transaction_count, unique_accounts, normalized metrics, composite_score, contribution breakdown, and concentration_ratio. Function signature: top_non_fungible_tokens_erc(window_hours INT DEFAULT 72, result_limit INT DEFAULT 50, exclusion_threshold NUMERIC DEFAULT 1.0). Methodology: https://docs.hgraph.com/hedera-stats-wip/top-50-non-fungible-tokens'),
